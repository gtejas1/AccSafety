...
def create_trail_dash(server, prefix="/trail/"):
    ...
    login_page_layout = dbc.Container([
        dbc.Row([
            dbc.Col([
                ...
                dcc.Input(id="trail-username", type="text", placeholder="Username", className="form-control mb-2"),
                dcc.Input(id="trail-password", type="password", placeholder="Password", className="form-control mb-2"),
                dbc.Button("Login", id="trail-login-button", color="primary", className="w-100"),
    ...
    summary_layout = dbc.Container([
        html.H2("Trail Counter Locations", className="text-center"),
    ...
    dashboard_layout = dbc.Container([
        dbc.Row([dbc.Col(html.H1(id="trail-dashboard-title", className="text-center"))]),
        dbc.Row([dbc.Col(dbc.Button("Back to Summary", href=f"{prefix}summary",
                                    color="secondary", className="mb-3"), width=12)]),
        dbc.Row([
            dbc.Col(dcc.DatePickerRange(id="trail-date-picker", display_format="MM-DD-YYYY"), width=6),
            dbc.Col(html.Div(
                html.A("Download Data", id="trail-download-link", href="", target="_blank",
    ...
    def _trail_download():
        ...
        df = pd.read_sql(
            "SELECT * FROM hr_traffic_data WHERE location_name = %(location)s",
    ...
    def trail_set_date_range(search):
        ...
        date_range = pd.read_sql(
            "SELECT MIN(date) AS min_date, MAX(date) AS max_date FROM hr_traffic_data WHERE location_name = %(l)s",
            ENGINE, params={"l": loc}
        )
    ...
    def trail_update_graphs(start_date, end_date, search):
        ...
        data = pd.read_sql(
            "SELECT * FROM hr_traffic_data WHERE location_name=%(l)s AND date BETWEEN %(s)s AND %(e)s",
