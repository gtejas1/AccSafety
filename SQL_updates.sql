...
WITH
/* ============================================================
   1) ECO aggregates (On-Street via ECO + Trails_Pilot_Counts)
   ============================================================ */
eco_ped_agg AS (
  SELECT
    e.location_name,
...
-- kept for compatibility; not used in final UNION
eco_both_agg AS (
  SELECT
    e.location_name,
    SUM(e.count)::bigint AS total_counts,
...
/* ============================================================
   3) FINAL UNION — mappings & labels per your request
   ============================================================ */

-- PILOT (ECO) → On-Street (sidewalk) · Pedestrian
SELECT
  e.location_name AS "Location",
  CASE
    WHEN e.total_hours <= 15 THEN '0-15hrs'
...
UNION ALL
-- PILOT (ECO) → On-Street (sidewalk/bike lane) · Bicyclist
SELECT
  e.location_name AS "Location",
  CASE
    WHEN e.total_hours <= 15 THEN '0-15hrs'
...
UNION ALL
-- STATEWIDE (Modeled) → Intersection · Pedestrian
SELECT
  swp.location_name AS "Location",
  swp.duration_bucket AS "Duration",
  swp.total_counts AS "Total counts",
...
UNION ALL
-- STATEWIDE (Modeled) → Intersection · Bicyclist
SELECT
  swb.location_name AS "Location",
  swb.duration_bucket AS "Duration",
  swb.total_counts AS "Total counts",
...
UNION ALL
-- (Optional) PILOT Trails shown under both modes (kept as before)
SELECT
  t.location_name AS "Location",
  CASE
    WHEN t.total_hours <= 15 THEN '0-15hrs'
